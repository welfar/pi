...
const btnLogin = async (email, password, userInfo, setUserInfo) => {
  const cookies = new Cookies();

  if (email !== "" && password !== "") {
    try {
      const resp = await axios.post(
        `${process.env.NEXT_PUBLIC_APP_SERVER_URL}/api/v1/auth/login`,
        {
          email,
          password,
        }
      ...
      window.location.href = `${process.env.NEXT_PUBLIC_APP_BASE_URL}/DomainSGI`;
    } catch (error) {
      console.log(error);
      alert("El usario o la contraseña no son validos");
    }
  } else {
    alert("El usario o la contraseña no son validos");
  }
...
const LoginForm = () => {
  ...
  console.log("mmmmmmmmmmm", userInfo);
        ...
        <div className="loginContainer__sesion">
          ...
          <div className="btn__container">
            <button
              ...
              onClick={(e) => {
                e.preventDefault();
                btnLogin(login.email, login.password, userInfo, setUserInfo);
              }}
            >
              Ingresar
            </button>
